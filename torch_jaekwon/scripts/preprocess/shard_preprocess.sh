#!/bin/bash
# Cluster-agnostic driver for ANY ShardPreprocessor subclass (run on the LOGIN
...
#       # shard_worker.sh with (module, TJ_PYTHON, TJ_REPO) as positional args.
#
...
#
# Strategy (see ShardPreprocessor docstring for the full rationale):
...
#   2. `python -m M count` -> leftover shards (login-safe). Exit if zero.
