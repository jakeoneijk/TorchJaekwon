...
class UtilVideo:
    ...
    def read_stream(
        ...
        for i, stream_key in enumerate(video_stream_key_list):
            ...
            assert video is not None, util.print(f"Can't read stream of {file_path}", msg_type='error')
            assert video.shape[0] >= video_stream_args_dict[stream_key]['frames_per_chunk'], util.print(f"Stream {stream_key} of {file_path} has less frames than expected. Expected: {video_stream_args_dict[stream_key]['frames_per_chunk']}, Actual: {video.shape[0]}", msg_type='error')
        ...
        for i, stream_key in enumerate(audio_stream_key_list):
            ...
            audio = util_audio.resample_audio(
                audio = audio, 
