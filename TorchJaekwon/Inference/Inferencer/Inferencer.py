...
class Inferencer():
    ...
    def get_model(self) -> Union[nn.Module,object]:
        return GetModule.get_model(self.h_params.model.class_name)
    ...
    def inference(self) -> None:
        ...
        for pretrained_path in pretrained_path_list:
            ...
            pretrained_name:str = UtilData.get_file_name_from_path(pretrained_path,False)
            meta_data_list:list = self.get_inference_meta_data_list()
            for meta_data in tqdm(meta_data_list,desc='inference data list'):
    ...
    def get_pretrained_path_list(self) -> list:
        if self.h_params.inference.pretrain_dir == '':
            self.h_params.inference.pretrain_dir = self.h_params.mode.config_name
        ...
        if self.h_params.inference.pretrain_module_name in ["all","last_epoch"]:
            ...
            pretrain_name_list = [self.h_params.inference.pretrain_module_name]
